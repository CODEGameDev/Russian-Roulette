...
function SWEP:draw3D2DHUD( )
	...
	cam.Start3D2D(att.Pos + ang:Forward() * -3 +ang:Up() * -4 + ang:Right() * 1, ang, 0.02)
		cam.IgnoreZ(true)
			ShadowText( 6-self:GetNWInt( "Order" ) .."/6", "CustomFont", 90, 80, 3, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
...
function SWEP:PrimaryAttack()   
	...
	if( self:GetNWInt( "Order" ) == self:GetNWInt( "Win" ) ) then
		...
		if SERVER then 
			self:SetNWInt( "Order", self:GetNWInt( "Order" ) + 1 ) 
...
function SWEP:SecondaryAttack()
	if SERVER then self.Owner:EmitSound( sound_next ) end
	self:DropRoulette()
end

function SWEP:DropRoulette()
	if SERVER then
		local roulette = ents.Create( 'entity_roulette' )
		local vec = self.Owner:GetAimVector()
		roulette:SetAngles( vec:Angle() )
		roulette:SetPos( self.Owner:EyePos() + vec * 20 )
		roulette:Spawn()
		roulette:Activate()
		...
		local phys = roulette:GetPhysicsObject()
		phys:SetVelocity( vec * 200 )
	end
end
